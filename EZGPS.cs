...
namespace EZGPS
{
    ...
    public class GPSUtilityCommands : MySessionComponentBase
    {
        ...
        public void NetworkHandler(ushort networkId, byte[] receivedData, ulong playerID, bool isFromServer)
        {
            ...
            List<String> words = new List<String>(data.text.Split(' '));
            ...
            switch (words[1])
            {
                ...
                case "i":
                    ...
                    if(match.Success && match.Index == 0)
                    {
                        ...
                        String[] importParams = importParamsString.Split(' ');
        ...
        {
            var tagSplit = new List<string>(tag.Trim().Split(' '));
            ...
            foreach (String word in tagSplit)
            {
                ...
                try
                {
                    doesContain &= Regex.IsMatch(g.Name, "(?<!\\S)" + cleanedWord + "(?!\\S)", RegexOptions.IgnoreCase) && cleanedWord.Length > 0;
                }
